...
### Main Script: `aiexec`
The entire application is a single bash script (E:\Archivio\Sviluppo\Scripts\aiexeccli\aiexec) that provides three operational modes:

1. **Simple Mode** (`ai` alias): Generates command without execution (always safe)
...
- `--install-aliases`: Create symlinks for ai/aix/aie
